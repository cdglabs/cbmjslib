...
(function() {
  ...
  fabric.Canvas = fabric.util.createClass(fabric.StaticCanvas, /** @lends fabric.Canvas.prototype */ {
    ...
    _rotateObject: function (x, y) {
      ...
      //t.target.angle = angle; //HS
      t.target.set('angle', angle);
    },
...
(function(global) {
  ...
  fabric.Object = fabric.util.createClass(/** @lends fabric.Object.prototype */ {
    ...
    get: function(property) {
      ...
      //HS : FIXME: avoid cycle of setting by frp / object
    acyclicSet: function(key, value) {
	this.isAcyclicSet = false
	this.set(key, value)
	this.setCoords()
	return this
    },
    ...
    set: function(key, value) {
      if (typeof key === 'object') {
	    ...
	    this._set(key, value);
	    //HS : if this is also a stream update stream definition belonging to owner (glendale object) to this value: FIXME
	    var owner = this.owner
	    if (owner && owner.streams && owner.streams[key] !== undefined && owner.streams[key] !== value && this.isAcyclicSet)
		owner.updateStreamDefinitionFromViewProperty(key, value)
	    this.isAcyclicSet = true
        }
      }
      return this;
    },
    ...
    render: function(ctx, noTransform) {
      ...
      //this._setShadow(ctx); //HS
      this.clipTo && fabric.util.clipContext(this, ctx);
      this._render(ctx, noTransform);
      this.clipTo && ctx.restore();
      //this._removeShadow(ctx); //HS
...
(function(global) {
  ...
  fabric.Image = fabric.util.createClass(fabric.Object, /** @lends fabric.Image.prototype */ {
    ...
    initialize: function(element, options) {
      ...
      //HS : FIXME: HACK FIXME! (I want stream named 'src' set this image's element src property
    acyclicSet: function(key, value) {
	if (value && key === 'src') {
	    this.getElement().setAttribute(key, value)
	    return this
	} else
	    return this.callSuper('acyclicSet', key, value)
    },
    ...
    _initConfig: function(options) {
      ...
      this.setOptions(options);
      this._setWidthHeight(options);
	if (this.crossOrigin) //HS: before there was no condition...
...
(function(global) {
  ...
  fabric.Text = fabric.util.createClass(fabric.Object, /** @lends fabric.Text.prototype */ {
    ...
    initialize: function(text, options) {
      ...
      //HS
      setTextWithWrap: function(t, canvas, borderObject, borderObjectMinHeight) {
  
	  var wrapMaxWidth = this.wrapMaxWidth == undefined ? 70 : this.wrapMaxWidth
	  var wrapMaxHeight = this.wrapMaxHeight
	  var textLines = t.split(this._reNewline)
	  var lineCounts = textLines.length
	  var numLines = 0
	  var res = ''
	  for (var i = 0; i < lineCounts; i++) {
	      var curr = textLines[i]
	      var currLen = curr.length
	      while (curr.length > wrapMaxWidth) {
	          numLines++
		  res += curr.substring(0, wrapMaxWidth) + '\n'
		  curr = curr.substring(wrapMaxWidth, curr.length)
	      }
	      res += curr
	      numLines++
	      if (wrapMaxHeight && (numLines >= wrapMaxHeight)) {
	          res += '...'
	          break
	      } else 
		  res += '\n'
	  ...
	  if (borderObject)
	      borderObject.height = Math.max(borderObjectMinHeight, numLines * 18)
	  this.text = res
},
...
(function() {
  ...
  fabric.IText = fabric.util.createClass(fabric.Text, fabric.Observable, /** @lends fabric.IText.prototype */ {
    ...
    _applyCharStylesGetWidth: function(ctx, _char, lineIndex, charIndex, decl) {
      ...
      //this._setShadow.call(styleDeclaration, ctx); //HS
